...
const sortReleases = (app, context, releases) => {
...
module.exports.findReleases = async ({ ref, context, config }) => {
  ...
  log({ app, context, message: JSON.stringify(sortedPublishedReleases.map(r => r.tag_name)) })
...
module.exports.updateRelease = ({ app, context, draftRelease, releaseInfo }) => {
