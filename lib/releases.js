...
module.exports.findReleases = async ({ app, context, config }) => {
  ...
  const sortedPublishedReleases = sortReleases(releases.filter((r) => !r.draft && (config['ignore-prereleases'] ? !r.prerelease : true)))
